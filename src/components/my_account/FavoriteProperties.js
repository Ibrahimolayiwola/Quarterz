...
const FavoriteProperties = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserListing = async () => {
      const propertyRef = collection(dataBase, "properties");
      const q = query(
        propertyRef,
  ...
  return (
    <div className="">
      <div className="xl:flex justify-between items-center py-6 text-slate-900 font-medium max-xl:w-[85%] max-xl:mx-auto xl:px-2">
        <p className="max-xl:mb-4 max-xl:border-b max-xl:border-b-slate-200 max-xl:p-2">
          My Properties
        </p>
        <p className="xl:space-x-10 max-xl:flex max-xl:flex-col">
          <span className=" max-xl:border-y max-xl:border-y-slate-200 max-xl:p-2">
            Date added
          </span>
          <span className=" max-xl:border-b max-xl:border-b-slate-200 max-xl:p-2">
            Actions
          </span>
          <span className=" max-xl:border-b max-xl:border-b-slate-200 max-xl:p-2">
            Delete
      ...
      >
        {myProperties &&
          myProperties.map((properties, index) => {
            return (
              <SwiperSlide className="" key={index}>
                <ul className="max-xl:w-[85%] max-xl:mx-auto">
                  {properties.map((property) => {
                    const {
                      data: { name, timeStamp, address, imageUrls },
                      id,
                    } = property;
                    return (
                      <li
                      ...
                      >
                        ...
                        <div className=" xl:space-x-12 xl:text-center xl:pr-8 max-xl:flex max-xl:flex-col max-xl:text-sm">
                          <Moment
                            format="MMM DD, YYYY"
                            className="max-xl:mt-3 max-xl:border-y max-xl:border-y-slate-200 max-xl:p-2"
                          >
                            {timeStamp?.toDate()}
                          </Moment>
                          <button className="max-xl:text-left  max-xl:border-b max-xl:border-b-slate-200 max-xl:p-2">
                            Edit
                          </button>
      ...
      <div className="myProperty-pagination flex justify-center items-center gap-2 cursor-pointer p-4"></div>
    </div>
  );
};
